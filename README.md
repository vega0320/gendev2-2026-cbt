...
- iPhone·iPad·Mac의 Chrome/Safari가 열린 상태에서도 15초마다 서버 풀이기록을 확인하고, 화면 복귀·온라인 전환 시 즉시 다시 병합함
- 강의록 추가문제(`-note-`)와 강의 연습문제(`-practice-`)를 포함한 전체 499개 문항 ID의 진행 기록을 서버가 보존함
- `app.js`와 `styles.css`에 내용 해시를 붙이고 재검증 캐시를 사용해 배포 직후에도 이전 화면 파일이 남지 않게 함
