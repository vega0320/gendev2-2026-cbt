...
def fact_intussusception(choice: str, qid: str) -> str:
    if has(choice, "공기 정복"):
        return "공기압으로 장중첩부를 근위부에서 원위부 방향의 반대로 밀어 풀며, 투시 또는 초음파로 회맹판 통과를 확인한다. 쇼크·천공·복막염이 없는 전형적 소아에서는 1차 치료다."
    if has(choice, "바륨관장"):
        return "바륨은 액체 압력으로 정복하면서 윤곽을 보여 주지만, 장이 천공되면 복강에 남아 심한 화학성 복막염을 일으킬 수 있다. 그래서 같은 비수술 정복이라도 공기나 수용성 액체가 선호된다."
    if has(choice, "초음파 유도 수기"):
        return "초음파 유도 생리식염수 관장은 정복술의 한 방식이지만, 복벽 바깥에서 손으로 종괴를 눌러 푸는 ‘수기 정복’은 압력 조절과 정복 확인이 어려워 표준 치료가 아니다."
    if has(choice, "복강경"):
        return "복강경은 장을 직접 잡아당겨 정복하고 Meckel 게실 같은 병적 선두점을 확인할 수 있다. 관장정복이 실패했거나 재발성·비전형적 장중첩에서 수술이 필요할 때 선택한다."
    if has(choice, "개복"):
        return "개복은 천공·복막염·혈역학 불안정 또는 비수술 정복 실패 때 필요하다. 장절제는 단순히 장중첩이 있다는 이유가 아니라, 직접 확인한 장이 괴사해 생존 가능성이 없을 때만 시행한다."
    if has(choice, "잠혈"):
...
def choice_fact(question: dict, index: int) -> str:
    ...
    qid = question["id"]
    exact = {
        "gendev2-16-2025-q086": [
            "5% 포도당은 중증 대사 위기에서 이화를 멈추기에 포도당 공급속도가 부족할 수 있다.",
            "10% 포도당은 충분한 포도당 공급속도로 단백질·지방 이화를 억제하며 전해질은 검사와 소변량에 맞춰 조절한다.",
            "Hartmann 용액의 lactate는 일부 유기산대사질환에서 해석과 대사 부담 문제가 있어 초기 열량 공급액으로 적합하지 않다.",
            "생리식염수는 쇼크 소생에는 쓰지만 포도당이 없어 지속 단독 투여하면 이화 상태를 멈추지 못한다.",
            "Ringer lactate는 포도당 열량을 제공하지 않고 lactate를 포함해 대사 위기의 초기 유지수액으로 부적절하다.",
        ],
        "gendev2-16-2021-q024": [
            "5% dextrose saline은 고암모니아혈증에서 단백질 이화를 충분히 억제할 만큼 높은 포도당 공급속도를 내기 어렵다.",
            "5% 포도당에 전해질을 더해도 포도당 농도가 낮아 650 µmol/L 고암모니아혈증의 역이화 치료에는 부족하다.",
            "10% 포도당으로 충분한 열량을 주어 단백질 이화를 즉시 막고, 동시에 암모니아 제거 치료를 병행한다.",
            "생리식염수는 탈수·쇼크 보충에는 유용하지만 포도당이 없어 암모니아 생성을 유발하는 이화를 억제하지 못한다.",
            "Hartmann 용액은 포도당을 공급하지 않으며 원인 미상의 대사 위기에서 lactate 함유액을 우선할 이유가 없다.",
        ],
        "gendev2-16-2020-q031": [
            "PAH 유전자검사는 원인 확정에 유용하지만 상승한 phenylalanine의 BH4 반응성을 즉시 기능적으로 평가하지 못한다.",
            "24시간 소변 아미노산은 고페닐알라닌혈증의 PAH 결핍과 BH4 결핍을 직접 구별하는 우선 검사가 아니다.",
            "소변 pterin 분석은 BH4 합성 결함 감별에 쓰지만 단독으로 치료 반응성을 보여 주지 않는다.",
            "BH4 부하 뒤 phenylalanine 감소를 보면 BH4 반응성 고페닐알라닌혈증을 기능적으로 확인하고 치료 선택에 연결할 수 있다.",
            "두 차례 상승이 확인된 상태에서 선별검사만 다시 반복하면 확진과 치료 시작이 지연된다.",
        ],
        "gendev2-17-2022-q018": [
            "앞숫구멍 폐쇄는 두개골의 신체 성숙 소견이지 운동·언어·사회성 같은 발달영역 수행이 아니다.",
            "출생체중의 2배는 신체 성장 속도를 나타내며 기능 획득을 평가하는 발달항목과 다르다.",
            "또래와 어울려 노는 능력은 사회성·정서 발달에 해당한다.",
            "BMI 백분위는 영양상태와 체격을 분류하는 성장지표다.",
            "머리둘레 백분위는 뇌 성장의 간접 지표이며 특정 기능을 수행하는 발달 이정표가 아니다.",
        ],
        "gendev2-15-2025-q083": [
            "고셔병은 glucocerebrosidase 결핍으로 국내 확대 신생아선별의 리소좀축적질환 항목에 포함된다.",
            "선지에 함께 적힌 니만-픽 A/B병과 파브리병은 모두 해당 확대 선별군에 포함되는 질환이다.",
            "Krabbe병은 galactocerebrosidase 결핍으로 2024년 추가 선별 항목에 포함된다.",
            "Tay-Sachs병은 hexosaminidase A 결핍 질환이지만 제시된 국내 무상 선별 6종에는 포함되지 않는다.",
            "Fabry병은 α-galactosidase A 결핍 리소좀축적질환으로 확대 선별 대상이다.",
        ],
        "gendev2-15-2022-q060": [
            "생후 12시간은 수유 전후 대사표지 변화가 충분하지 않아 위음성 가능성이 높다.",
            "24시간 검체는 조기 퇴원 때 사용할 수 있지만 일부 질환은 재검이 필요해 48시간 이후보다 덜 이상적이다.",
            "생후 60시간은 충분히 수유한 48~72시간 창에 들어가 만삭아 채혈 시기로 적절하다.",
            "생후 7일까지 미루면 양성 질환의 조기 확진·치료 시작이 늦어진다.",
            "생후 10일 채혈은 갈락토스혈증·선천갑상샘저하증 등 시간 민감 질환의 치료 기회를 더 지연시킨다.",
        ],
        "gendev2-17-2021-q019": [
            "6개월 영아는 서 있는 자세를 유지할 수 없어 선키 측정의 재현성이 매우 낮다.",
            "12개월에는 독립 보행과 협조가 불안정하므로 누운키를 계속 사용한다.",
            "18개월도 몸을 곧게 세워 표준 자세를 유지하기 어려워 선키 전환 전이다.",
            "24개월 미만은 누운키, 24개월부터는 선키를 표준으로 쓰는 구분이 맞다.",
            "36개월까지 누운키만 사용하면 2세부터 적용하는 표준 선키 전환보다 1년 늦다.",
        ],
        "gendev2-17-2020-q030": [
            "D로 시작하면 가장 성숙한 radial-digital grasp를 첫 단계에 놓으므로 시간 순서가 거꾸로다.",
            "D→U는 성숙한 요측 손가락 잡기 뒤 미성숙 척측 손바닥 잡기로 되돌아가 불가능하다.",
            "P 다음 D까지는 성숙 방향이지만 마지막에 U로 퇴행하도록 배열해 틀렸다.",
            "U에서 D로 건너뛴 뒤 P로 되돌아가 중간 단계의 위치가 바뀌었다.",
            "ulnar-palmar(U)에서 radial-palmar(P), radial-digital(D)로 가는 척측→요측·손바닥→손가락 순서가 맞다.",
        ],
    }
    if qid in exact:
        return exact[qid][index]
